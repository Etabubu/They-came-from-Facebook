...
package org.amfphp.utils
{
	...
	public class RawAMFService extends EventDispatcher
	{
		...
		/**
		 * Creates the Credentials header that contains the <code>userid</code> and the <code>password</code>
		 * 
		 * @see http://osflash.org/documentation/amf/envelopes/remoting/headers#credentials
		 * @see http://osflash.org/documentation/amf/astypes#x03object
		 */
		private function createCredentialsHeader(data:ByteArray):void
		{
			data.writeByte(0x00);
			data.writeByte(11);						// Set the length (in bytes) of the header name
			data.writeUTFBytes("Credentials");		// Set the header name

			data.writeByte(0x00);					// Set required to false

			data.writeByte(0x00);
			data.writeByte(0x00);
			data.writeByte(0x00);
			data.writeByte(0x02);					// Set the length of the header

			data.writeByte(0x03);					// Set the body type code to Object 
			
			data.writeUTF("userid");				// Set the userid key
			data.writeByte(0x02);					// Set the type code to String 
			data.writeUTF("danny");					// Set the userid value
			
			data.writeUTF("password");				// Set the password key
			data.writeByte(0x02);					// Set the type code to String
			data.writeUTF("test123");				// Set the password value
			
			data.writeByte(0x00);					
			data.writeByte(0x00);
			data.writeByte(0x09);					// Header end
		}
		...
		private function readData(event:Event):void
		{
			...
			else
			{
				...
				fault = new Fault("INVALID_AMF_MESSAGE", "Invalid AMF message", errorMessage);
				sendEvent = new FaultEvent(FaultEvent.FAULT, false, true, fault);
					//dispatchEvent(fe);
			}
